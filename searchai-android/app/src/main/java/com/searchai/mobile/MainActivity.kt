...
@Composable fun SearchAiScreen(open:(SearchSource,String)->Unit){
 var query by remember{ mutableStateOf("") }; var searched by remember{ mutableStateOf(false) }; var showSettings by remember{mutableStateOf(false)}
 ...
 Scaffold(topBar={TopAppBar(title={Text("SearchAI ✨")},actions={TextButton(onClick={showSettings=true}){Text("Updates")}})}){ pad->
  LazyColumn(Modifier.padding(pad).padding(20.dp).fillMaxSize(),verticalArrangement=Arrangement.spacedBy(12.dp)){
   ...
   item{OutlinedTextField(query,{query=it},Modifier.fillMaxWidth(),label={Text("What are you looking for?")},minLines=2)}
   item{Button(onClick={if(!Safety.isSensitiveAction(query)){ {searched=true} } else {{}}},enabled=query.isNotBlank(),modifier=Modifier.fillMaxWidth()){Text("Search Everywhere")}}
   if(Safety.isSensitiveAction(query)) item{Text("SearchAI can help you research and compare, but it won't make purchases, payments, bids, or messages for you.")}
